
HandyNotes_ShadowlandsDB = {
	["profileKeys"] = {
		["Damnuspoop - Mazrigos"] = "Default",
		["Shirang - Mazrigos"] = "Default",
		["Dannidan - Mazrigos"] = "Default",
		["Dehlicia - Mazrigos"] = "Default",
		["Merwarr - Mazrigos"] = "Default",
		["Ytoh - Mazrigos"] = "Default",
		["Fartnipplebo - Lightbringer"] = "Default",
		["Yato - Mazrigos"] = "Default",
		["Arthees - Mazrigos"] = "Default",
		["Nitus - Tarren Mill"] = "Default",
	},
	["profiles"] = {
		["Default"] = {
			["icon_display_pet_battles"] = false,
			["icon_display_misc"] = true,
			["icon_scale_rares"] = 0.7000000476837158,
			["icon_alpha_treasures"] = 1,
			["icon_scale_treasures"] = 0.7000000476837158,
			["icon_alpha_rares"] = 1,
			["icon_scale_pet_battles"] = 0.300000011920929,
			["icon_display_rift_portal"] = true,
			["icon_alpha_pet_battles"] = 0.3499999940395355,
			["icon_display_rares"] = false,
		},
	},
}
